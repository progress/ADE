...
DEFINE       OUTPUT PARAMETER pcObjectTypeCode      AS CHARACTER    NO-UNDO.
...
ON WINDOW-CLOSE OF FRAME gDialog /* Lookup SmartObjects */
DO:
...
DO:
    IF AVAILABLE ryc_smartObject THEN
        ASSIGN pcObjectFilename = ryc_smartObject.object_filename
               pcObjectTypeCode = gsc_object_type.object_type_code
               pdObjectTypeObj  = gsc_object_type.object_type_obj
               .
    ELSE
        ASSIGN pcObjectFilename = "":U
               pcObjectTypeCode = "":U
...
------------------------------------------------------------------------------*/
    ...
    ASSIGN hCombo        = coObjectType:HANDLE IN FRAME {&FRAME-NAME}
           cLipString    = "":U
           cQueryPrepare = " FOR EACH gsc_object_type WHERE ":U
                         + (IF pcObjectTypeWhere EQ "":U THEN " TRUE ":U ELSE pcObjectTypeWhere)
                         + " NO-LOCK ":U
           .

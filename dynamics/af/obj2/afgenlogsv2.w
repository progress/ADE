...
DEFINE TEMP-TABLE ttSection     NO-UNDO
    ...
    INDEX idxOrder      AS PRIMARY UNIQUE
        tSectionOrder
    .
...
&Scoped-Define ENABLED-OBJECTS edMessageView 
&Scoped-Define DISPLAYED-OBJECTS edMessageView 
...
------------------------------------------------------------------------------*/
    ...
    DEFINE VARIABLE iErrorImageIndex    AS INTEGER                      NO-UNDO.
    ...
    ASSIGN iImageIndex = DYNAMIC-FUNCTION("loadImage" IN TARGET-PROCEDURE,  "ry/img/treefile.ico":U).
    ASSIGN iSelectedImageIndex = DYNAMIC-FUNCTION("loadImage" IN TARGET-PROCEDURE, "ry/img/treefils.ico":U).
    ASSIGN iErrorImageIndex = DYNAMIC-FUNCTION("loadImage" IN TARGET-PROCEDURE,  "ry/img/error24.ico":U).
    ...
    FIND FIRST ttSection WHERE
               ...
               NO-ERROR.
    IF NOT AVAILABLE ttSection THEN
    DO:
        FIND LAST lbSection NO-ERROR.

        CREATE ttSection.
        ASSIGN ttSection.tSectionName  = ttErrorLog.tSection
               ...
               .
        ASSIGN chNode = chTreeview:Nodes:ADD(cParentKey, /* Parent Key (optional) */
        ...
        IF ttErrorLog.tAction EQ ttErrorLog.tSection OR ttErrorLog.tAction EQ "":U THEN
            ASSIGN chNode:Tag = STRING(ROWID(ttErrorLog)).
        ELSE
            ASSIGN chNode:Tag = "":U.
    END.    /* n/a section */

    /* Action not the same as the saection. */
    IF ttErrorLog.tAction NE ttErrorLog.tSection AND ttErrorLog.tAction NE "":U THEN
    DO:    
        FIND FIRST ttAction WHERE
                   ttAction.tActionName   = ttErrorLog.tAction      AND
        ...
        DO:
            ...
            CREATE ttAction.
            ASSIGN ttAction.tActionName   = ttErrorLog.tAction
             ...
             ASSIGN chNode = chTreeview:Nodes:ADD(cParentKey, /* Parent Key (optional) */
                                                  ...
                                                  iImageIndex            ,  /* Normal Image         */
                                                  iSelectedImageIndex    ). /* Selected Image       */        
             IF ttErrorLog.tResult EQ ttErrorLog.tAction OR ttErrorLog.tResult EQ "":U THEN
                 ASSIGN chNode:Tag = STRING(ROWID(ttErrorLog)).
             ELSE
                 ASSIGN chNode:Tag = "":U. 
        END.    /* n/a Action */
    ...
    IF ttErrorLog.tResult NE ttErrorLog.tAction AND ttErrorLog.tResult NE "":U THEN
    DO:
        FIND LAST lbResult NO-ERROR.

        CREATE ttResult.
        ASSIGN ttResult.tResultName  = ttErrorLog.tResult
        ...
        ASSIGN chNode = chTreeview:Nodes:ADD(cParentKey, /* Parent Key (optional) */
                                             ...
                                             (IF ttErrorLog.tErrorType EQ "ERROR":U THEN iErrorImageIndex ELSE iImageIndex),  /* Normal Image         */
                                             (IF ttErrorLog.tErrorType EQ "ERROR":U THEN iErrorImageIndex ELSE iSelectedImageIndex)).  /* Normal Image         */
...
------------------------------------------------------------------------------*/
    ...
    DO WITH FRAME {&FRAME-NAME}:
        ASSIGN hTreeview:ROW          = 1
               hTreeview:COLUMN       = 1
               hTreeview:HEIGHT-CHARS = FRAME {&FRAME-NAME}:HEIGHT-CHARS
               hTreeview:WIDTH-CHARS  = FRAME {&FRAME-NAME}:WIDTH-CHARS * 0.40
               
               edMessageView:ROW          = 1
               edMessageView:COLUMN       = hTreeview:COLUMN + hTreeview:WIDTH-CHARS + 0.3
               edMessageView:HEIGHT-CHARS = FRAME {&FRAME-NAME}:HEIGHT-CHARS
...
------------------------------------------------------------------------------*/
    ...
    IF phNode:Tag NE "":U THEN
    DO WITH FRAME {&FRAME-NAME}:
        ASSIGN rErrorLog = TO-ROWID(phNode:tag).

        FIND FIRST ttErrorLog WHERE 
                   ROWID(ttErrorLog) = rErrorLog
                   NO-ERROR.
        IF AVAILABLE ttErrorLog THEN
        DO:
            ASSIGN edMessageView:SCREEN-VALUE = "":U.
            edMessageView:INSERT-STRING( "Short Message:~n":U
                                   + FILL("=":U, 14) + "~n":U
                                   ...
                                   + FILL("=":U, 18) + "~n":U
                                   + "Error Type: ":U + ttErrorLog.tErrorType + "~n":U
